...
class TagAwareAdapter implements \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Adapter\TagAwareAdapterInterface, \Prokerala\Astrology\Vendor\Symfony\Contracts\Cache\TagAwareCacheInterface, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\PruneableInterface, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\ResettableInterface, \Psr\Log\LoggerAwareInterface
{
    ...
    public function __construct(\Prokerala\Astrology\Vendor\Symfony\Component\Cache\Adapter\AdapterInterface $itemsPool, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Adapter\AdapterInterface $tagsPool = null, float $knownTagVersionsTtl = 0.15)
    {
        $this->pool = $itemsPool;
        $this->tags = $tagsPool ?: $itemsPool;
        $this->knownTagVersionsTtl = $knownTagVersionsTtl;
        $this->createCacheItem = \Closure::bind(static function ($key, $value, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\CacheItem $protoItem) {
        ...
        $this->setCacheItemTags = \Closure::bind(static function (\Prokerala\Astrology\Vendor\Symfony\Component\Cache\CacheItem $item, $key, array &$itemTags) {
            $item->isTaggable = \true;
            if (!$item->isHit) {
                return $item;
            }
            if (isset($itemTags[$key])) {
                foreach ($itemTags[$key] as $tag => $version) {
                    $item->metadata[\Prokerala\Astrology\Vendor\Symfony\Component\Cache\CacheItem::METADATA_TAGS][$tag] = $tag;
                }
                unset($itemTags[$key]);
            } else {
                $item->value = null;
        ...
        $this->getTagsByKey = \Closure::bind(static function ($deferred) {
            ...
            foreach ($deferred as $key => $item) {
                $tagsByKey[$key] = $item->newMetadata[\Prokerala\Astrology\Vendor\Symfony\Component\Cache\CacheItem::METADATA_TAGS] ?? [];
                $item->metadata = $item->newMetadata;
            }
            return $tagsByKey;
        }, null, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\CacheItem::class);
        $this->invalidateTags = \Closure::bind(static function (\Prokerala\Astrology\Vendor\Symfony\Component\Cache\Adapter\AdapterInterface $tagsAdapter, array $tags) {
            foreach ($tags as $v) {
                $v->expiry = 0;
    ...
    public function invalidateTags(array $tags)
    {
        ...
        $invalidatedTags = [];
        foreach ($tags as $tag) {
            \Prokerala\Astrology\Vendor\Symfony\Component\Cache\CacheItem::validateKey($tag);
