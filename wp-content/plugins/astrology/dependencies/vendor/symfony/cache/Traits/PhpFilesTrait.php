    ...
    protected function doFetch(array $ids)
    {
        ...
        foreach ($ids as $id) {
            if (null === ($value = $this->values[$id] ?? null)) {
                ...
                $values[$id] = $value;
            } elseif (!$value instanceof \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Traits\LazyValue) {
                $values[$id] = $value();
            ...
            foreach ($missingIds as $k => $id) {
                try {
                    ...
                    if (isset(self::$valuesCache[$file])) {
                        ...
                        $this->values[$id] = new \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Traits\LazyValue($file);
                    }
                    if ($now >= $expiresAt) {
    ...
    protected function doHave($id)
    {
        ...
        try {
            ...
            if (isset(self::$valuesCache[$file])) {
                ...
                $value = new \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Traits\LazyValue($file);
            }
        } catch (\ErrorException $e) {
            return \false;
        } finally {
    ...
    protected function doSave(array $values, int $lifetime)
    {
        ...
        foreach ($values as $key => $value) {
            ...
            if (null === $value) {
                ...
                try {
                    $value = \Prokerala\Astrology\Vendor\Symfony\Component\VarExporter\VarExporter::export($value, $isStaticValue);
                } catch (\Exception $e) {
                    throw new \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache key "%s" has non-serializable "%s" value.', $key, \is_object($value) ? \get_class($value) : 'array'), 0, $e);
                }
                ...
                throw new \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('Cache key "%s" has non-serializable "%s" value.', $key, \gettype($value)));
            } else {
                $value = \var_export($value, \true);
            }
        ...
        if (!$ok && !\is_writable($this->directory)) {
            throw new \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Exception\CacheException(\sprintf('Cache directory is not writable (%s).', $this->directory));
        }
        return $ok;
    }
    /**
