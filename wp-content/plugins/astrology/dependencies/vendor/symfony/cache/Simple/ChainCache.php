...
@\trigger_error(\sprintf('The "%s" class is deprecated since Symfony 4.3, use "%s" and type-hint for "%s" instead.', \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Simple\ChainCache::class, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Adapter\ChainAdapter::class, \Prokerala\Astrology\Vendor\Symfony\Contracts\Cache\CacheInterface::class), \E_USER_DEPRECATED);
/**
 ...
 */
class ChainCache implements \Psr\SimpleCache\CacheInterface, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\PruneableInterface, \Prokerala\Astrology\Vendor\Symfony\Component\Cache\ResettableInterface
{
    private $miss;
    private $caches = [];
    ...
    {
        if (!$caches) {
            throw new \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException('At least one cache must be specified.');
        }
        foreach ($caches as $cache) {
            if (!$cache instanceof \Psr\SimpleCache\CacheInterface) {
                throw new \Prokerala\Astrology\Vendor\Symfony\Component\Cache\Exception\InvalidArgumentException(\sprintf('The class "%s" does not implement the "%s" interface.', \get_class($cache), \Psr\SimpleCache\CacheInterface::class));
            }
        }
    ...
    public function prune()
    {
        $pruned = \true;
        foreach ($this->caches as $cache) {
            if ($cache instanceof \Prokerala\Astrology\Vendor\Symfony\Component\Cache\PruneableInterface) {
    ...
    {
        foreach ($this->caches as $cache) {
            if ($cache instanceof \Prokerala\Astrology\Vendor\Symfony\Contracts\Service\ResetInterface) {
                $cache->reset();
            }
        }
    }
