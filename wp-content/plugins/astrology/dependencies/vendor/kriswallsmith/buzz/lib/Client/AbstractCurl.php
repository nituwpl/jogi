 ...
 */
abstract class AbstractCurl extends \Prokerala\Astrology\Vendor\Buzz\Client\AbstractClient
{
    private $handles = [];
    ...
    protected function createHandle()
    {
        ...
        if (\false === $curl) {
            throw new \Prokerala\Astrology\Vendor\Buzz\Exception\ClientException('Unable to create a new cURL handle');
        }
        return $curl;
    }
    /**
     ...
     * @param resource $curl
     */
    protected function prepare($curl, \Psr\Http\Message\RequestInterface $request, \Prokerala\Astrology\Vendor\Buzz\Configuration\ParameterBag $options) : \Prokerala\Astrology\Vendor\Buzz\Message\ResponseBuilder
    {
        ...
        $responseBuilder = new \Prokerala\Astrology\Vendor\Buzz\Message\ResponseBuilder($this->responseFactory);
    ...
    {
        $options = [\CURLOPT_CUSTOMREQUEST => $request->getMethod(), \CURLOPT_URL => $request->getUri()->__toString(), \CURLOPT_HTTPHEADER => \Prokerala\Astrology\Vendor\Buzz\Message\HeaderConverter::toBuzzHeaders($request->getHeaders())];
     ...
     * @param resource $curl
     */
    private function setOptionsFromParameterBag($curl, \Prokerala\Astrology\Vendor\Buzz\Configuration\ParameterBag $options) : void
    {
     ...
     */
    protected function parseError(\Psr\Http\Message\RequestInterface $request, int $errno, $curl) : void
    {
        switch ($errno) {
            case \CURLE_OK:
            ...
            case \CURLE_SSL_CONNECT_ERROR:
                throw new \Prokerala\Astrology\Vendor\Buzz\Exception\NetworkException($request, \curl_error($curl), $errno);
            case \CURLE_ABORTED_BY_CALLBACK:
                throw new \Prokerala\Astrology\Vendor\Buzz\Exception\CallbackException($request, \curl_error($curl), $errno);
            default:
                throw new \Prokerala\Astrology\Vendor\Buzz\Exception\RequestException($request, \curl_error($curl), $errno);
        }
    }
    private function getProtocolVersion(\Psr\Http\Message\RequestInterface $request) : int
    {
