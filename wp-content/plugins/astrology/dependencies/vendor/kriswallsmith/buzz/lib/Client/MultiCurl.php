...
class MultiCurl extends \Prokerala\Astrology\Vendor\Buzz\Client\AbstractCurl implements \Prokerala\Astrology\Vendor\Buzz\Client\BatchClientInterface, \Prokerala\Astrology\Vendor\Buzz\Client\BuzzClientInterface
{
    private $queue = [];
    private $curlm;
    /**
     ...
     *     };
     */
    public function sendAsyncRequest(\Psr\Http\Message\RequestInterface $request, array $options = []) : void
    {
    ...
    public function sendRequest(\Psr\Http\Message\RequestInterface $request, array $options = []) : \Psr\Http\Message\ResponseInterface
    {
        ...
        $options = $options->add(['callback' => function (\Psr\Http\Message\RequestInterface $request, \Psr\Http\Message\ResponseInterface $response = null, \Prokerala\Astrology\Vendor\Buzz\Exception\ClientException $e = null) use(&$responseToReturn, $originalCallback) {
    ...
    protected function configureOptions(\Prokerala\Astrology\Vendor\Symfony\Component\OptionsResolver\OptionsResolver $resolver) : void
    {
        ...
        $resolver->setDefault('callback', function (\Psr\Http\Message\RequestInterface $request, \Psr\Http\Message\ResponseInterface $response = null, \Prokerala\Astrology\Vendor\Buzz\Exception\ClientException $e = null) {
        });
    ...
    private function addToQueue(\Psr\Http\Message\RequestInterface $request, \Prokerala\Astrology\Vendor\Buzz\Configuration\ParameterBag $options) : array
    {
    ...
    private function initMultiCurlHandle() : void
    {
        ...
        if (\false === $this->curlm) {
            throw new \Prokerala\Astrology\Vendor\Buzz\Exception\ClientException('Unable to create a new cURL multi handle');
        }
    ...
    private function initQueue() : void
    {
        foreach ($this->queue as $i => $queueItem) {
            ...
            if ($this->serverPushSupported && $options->get('use_pushed_response') && $this->hasPushResponse($request->getUri()->__toString())) {
                ...
                $response = (new \Prokerala\Astrology\Vendor\Buzz\Message\ResponseBuilder($this->responseFactory))->getResponseFromRawInput($data['content'], $data['headerSize']);
