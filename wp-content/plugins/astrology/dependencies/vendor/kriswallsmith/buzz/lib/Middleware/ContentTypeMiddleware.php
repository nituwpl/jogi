...
class ContentTypeMiddleware implements \Prokerala\Astrology\Vendor\Buzz\Middleware\MiddlewareInterface
{
    /**
    ...
    {
        $resolver = new \Prokerala\Astrology\Vendor\Symfony\Component\OptionsResolver\OptionsResolver();
    ...
    private function isJson(\Psr\Http\Message\StreamInterface $stream) : bool
    {
        $stream->rewind();
        \json_decode($stream->getContents());
        return \JSON_ERROR_NONE === \json_last_error();
    }
    private function isXml(\Psr\Http\Message\StreamInterface $stream) : bool
    {
        $stream->rewind();
