...
final class ResponseBuilder
{
    ...
    {
        if (!$responseFactory instanceof \Prokerala\Astrology\Vendor\Http\Message\ResponseFactory && !$responseFactory instanceof \Psr\Http\Message\ResponseFactoryInterface) {
            throw new \Prokerala\Astrology\Vendor\Buzz\Exception\InvalidArgumentException('First parameter to ResponseBuilder must be a response factory');
        }
        $this->responseFactory = $responseFactory;
        $this->response = $responseFactory->createResponse();
    }
    public function getResponseFromRawInput(string $raw, int $headerSize) : \Psr\Http\Message\ResponseInterface
    {
    ...
    public function setStatus(string $input) : void
    {
        ...
        if (\count($parts) < 2 || 0 !== \strpos(\strtolower($parts[0]), 'http/')) {
            throw new \Prokerala\Astrology\Vendor\Buzz\Exception\InvalidArgumentException(\sprintf('"%s" is not a valid HTTP status line', $input));
        }
    ...
    public function parseHttpHeaders(array $headers) : void
    {
        ...
        } catch (\Prokerala\Astrology\Vendor\Buzz\Exception\InvalidArgumentException $e) {
